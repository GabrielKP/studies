define(["component/Pages"], function (Pages) {
  class _CueAssociation {
    ...
    response_time_limit = 10000; // response threshold
    response_timer = null;
    ...
    show_cue_new_textbox() {
      $("#qinput").val("").hide().prop("disabled", true);
      this.fade_cue(this.word_text.toUpperCase());
      this.ready_word_variables();
      this.listening = false; // Disable listening until textbox is shown
      ...
      this.response_timer = setTimeout(() => {
        // only show textbox after 10 seconds
        $("#qinput").prop("disabled", false).show().focus();
        this.key_start_time = new Date().getTime();
        this.listening = true;
